...
func (config *PrometheusConfig) NewEmitter() (metric.Emitter, error) {
	...
	tasksWaitingDuration := prometheus.NewHistogramVec(prometheus.HistogramOpts{
		...
		Subsystem: "tasks",
		Name:      "wait_duration",
		Help:      "Elasped time waiting for execution",
...
func (emitter *PrometheusEmitter) Emit(logger lager.Logger, event metric.Event) {
	...
	case "tasks waiting":
		emitter.tasksWaiting.WithLabelValues(event.Attributes["teamId"], event.Attributes["workerTags"], event.Attributes["platform"]).Set(event.Value)
	case "tasks waiting duration":
		logger.Info(fmt.Sprintf("Prometheus is emitting the tasks waiting duration metric, of value: %f", event.Value))
		emitter.tasksWaitingDuration.WithLabelValues(event.Attributes["teamId"], event.Attributes["workerTags"], event.Attributes["platform"]).Observe(event.Value)
	case "build finished":
